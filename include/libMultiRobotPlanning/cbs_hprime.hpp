...
class CBS {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    start.cost = 0;
    start.id = 0;
    // std::vector<PlanResult<State, Action, Cost>> trace_paths;
    // 3D vector, initial shape: [0][dimX][dimY]
    std::vector<std::vector<std::vector<int>>> trace_paths(
        0, std::vector<std::vector<int>>(m_env.getDimX(),
                                         std::vector<int>(m_env.getDimY(), 0)));
    for (size_t i = 0; i < initialStates.size(); ++i) {
      ...
      // std::cout << "trace_paths.size():" << trace_paths.size() << std::endl;

      bool success =
          lowLevel.search(initialStates[i], start.solution[i], trace_paths);
      for (size_t t = 0; t < start.solution[i].states.size(); t++) {
        // std::cout << start.solution[i];

        int x = start.solution[i].states[t].first.x;
        int y = start.solution[i].states[t].first.y;
        if (t >= trace_paths.size()) {
          trace_paths.resize(t + 1, std::vector<std::vector<int>>(
                                        m_env.getDimX(),
                                        std::vector<int>(m_env.getDimY(), 0)));
        }
      ...
      // trace_paths <- start.solution[i]
      // trace_paths.push_back(start.solution[i]);
      // for (size_t t = 0; t < trace_paths[i].states.size(); t++) {
      //   std::cout << "Agent: " << i
      //             << " State: " << trace_paths[i].states[t].first <<
      //             std::endl;
      // }  // {example} Agent: 45 State: 6: (19,30)
    ...
    while (!open.empty()) {
      ...
      for (const auto& c : constraints) {
        ...
        LowLevelSearch_t lowLevel(llenv);
        bool success =
            lowLevel.search(initialStates[i], newNode.solution[i], trace_paths);
        // trace_paths <- newNode.solution[i]
        // trace_paths.push_back(newNode.solution[i]);
        for (size_t t = 0; t < newNode.solution[i].states.size(); t++) {
          int x = newNode.solution[i].states[t].first.x;
          int y = newNode.solution[i].states[t].first.y;
          if (t >= trace_paths.size()) {
            trace_paths.resize(
                t + 1,
                std::vector<std::vector<int>>(
                    m_env.getDimX(), std::vector<int>(m_env.getDimY(), 0)));
          }
 ...
 private:
  Environment& m_env;
  typedef AStarHPrime<State, Action, Cost, LowLevelEnvironment>
      LowLevelSearch_t;
};
